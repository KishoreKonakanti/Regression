...
class FetchDetails(threading.Thread):
    ...
    def run(self,skipLines = 9):
        ...
        for link in linkFile.readlines():
            link = link.strip()
            now = time.time()
            print('Current Site#%d: %s'%(cnt,link))
            print('Time elapsed:%d seconds'%(now - start_time))
            cnt += 1
            link = self.transform(link)
            if link not in self.crawledList and link is None:
                continue
...
try:
    ...
    for DOMAIN in DOMAIN_list:
        ...
        current = FetchDetails(DOMAIN, writer)
        thread_set.append(current)
        current.run()        
        print('Started a thread for %s'%DOMAIN)
    ...
    print('  {0} has been raised'.format(e))
finally:
